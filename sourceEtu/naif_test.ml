...
(*
  ...
  Post-conditions : le résultat est strictement positif et correspond à la position de la première occurrence
  Exceptions :
...
let%test _ = try let _ = index 'd' ['a';'b';'c'] in false with Failure "Lettre incorrecte" -> true
let%test _ = try let _ = index 'a' [] in false with Failure "Lettre incorrecte" -> true
...
(*
  sequence_encoder_lettre : encodage -> char -> int list
  Encode une lettre sous forme d’une séquence d'appuis de touches.
  Paramètre code : liste associative représentant l'encodage (touche, liste de lettres)
  Paramètre lettre : caractère à encoder
  Résultat : liste d’entiers représentant les appuis nécessaires, suivie de 0 (pause)
  Pré-conditions :
...
let%test _ = try let _ = sequence_encoder_lettre t9_map '%' in false with Failure "Liste encodage incorrecte" -> true
let%test _ = try let _ = sequence_encoder_lettre [] 'a' in false with Failure "Liste encodage incorrecte" -> true
let%test _ = try let _ = sequence_encoder_lettre [(2, ['b'; 'c'])] 'a' in false with Failure "Liste encodage incorrecte" -> true

(*
  encoder_mot : encodage -> string -> int list
  Encode un mot entier en séquence d'appuis sur les touches.
  Paramètre code : encodage, une liste associative (touche, lettres associées)
  Paramètre mot : chaîne de caractères à encoder
  Résultat : liste d’entiers représentant la séquence complète d'appuis avec les pauses
  Pré-conditions :
...
let%test _ = try let _ = encoder_mot t9_map "ab%" in false with Failure "Liste encodage incorrecte" -> true
...
(*
  ...
  Identifie la lettre à partir d’une touche et du nombre d'appuis.
  Paramètre encodage : liste associative (touche, liste de lettres associées)
  Paramètre saisie : couple (touche, nombre d'appuis effectuées)
  Résultat : la lettre correspondante à cette saisie
  Pré-conditions :
    - La touche doit exister dans l’encodage
    - Le nombre d'appuis est >= 1 et <= nombre de lettres associées à cette touche
    - L'encodage n’est pas vide
  Post-conditions :
    - Retourne la lettre à la ième position dans la liste associée à la touche
  Exceptions :
...
(*
  ...
  Pré-conditions :
    ...
    - La suite de touches suit strictement le protocole :
      - 0 sépare deux lettres différentes (ou une en fin de mot)
  ...
  Post-conditions :
    - La chaîne retournée contient exactement une lettre pour chaque séquence d'appuis valides
  ...
  let aggreger acc el =
    match fst acc with
      ...
      | Some (touche, nb) -> if (el = 0) then (None, ((snd acc) ^ String.make 1 (decoder_lettre encodage (touche, nb)))) 
        (* initialement : (String.cat (snd acc) (String.make 1 (decoder_lettre encodage (touche, nb))))
           cependant : cette formulation ne fonctionnait pas sur la version OCAML d'un membre du binôme *)
